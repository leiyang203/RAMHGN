...
def train():
    ...
    if args.dataset in ['Kdataset', 'Bdataset']:
        ...
        df = pd.read_csv(get_Fdataset_path('drug_disease.csv'), header=None).values
    ...
    for (train_pos_idx, test_pos_idx), (train_neg_idx, test_neg_idx) in zip(kf.split(data_pos),
        ...
        pos_weight = len(train_neg_idx[0]) / len(train_pos_idx[0]) * args.pos_weight_scale
        criterion = th.nn.BCEWithLogitsLoss(pos_weight=th.tensor(pos_weight))
        print('Loss pos weight: {:.3f}'.format(pos_weight))
