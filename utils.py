...
def m2v(g, metapaths, device=None, emb_dim=128):
    """Learn MetaPath2Vec embeddings from one or more metapaths."""

    if device is None:
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    if len(metapaths) > 0 and isinstance(metapaths[0], str):
        metapaths = [metapaths]

    drug_emb_list = []
    disease_emb_list = []
    for metapath in metapaths:
        model = MetaPath2Vec(g, metapath, window_size=3, emb_dim=emb_dim)
        model.to(device)

        src_ntype = g.to_canonical_etype(metapath[0])[0]
        dataloader = DataLoader(torch.arange(g.num_nodes(src_ntype)), batch_size=128,
        ...
        if 'drug' not in model.local_to_global_nid or 'disease' not in model.local_to_global_nid:
            continue
        ...
        drug_emb_list.append(model.node_embed(drug_nids).detach())
        disease_emb_list.append(model.node_embed(disease_nids).detach())

    if len(drug_emb_list) == 0 or len(disease_emb_list) == 0:
        raise ValueError('No valid metapath can provide both drug and disease embeddings.')

    return torch.stack(drug_emb_list, dim=1), torch.stack(disease_emb_list, dim=1)
